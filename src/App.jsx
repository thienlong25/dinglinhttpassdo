...
function isExactIdMatch(idValue, queryValue) {
  const query = String(queryValue || "").trim().toLowerCase();
  if (!query) return true;
  return String(idValue || "").trim().toLowerCase() === query;
}
...
function ShopView({ buyerIg, setBuyerIg, buyerFullName, setBuyerFullName, buyerPhone, setBuyerPhone, buyerOldAddress, setBuyerOldAddress, phoneError, addressError, showBuyerForm, setShowBuyerForm, search, setSearch, products, now, closedOrders, hasBuyerPhone, showClosedOrders, setShowClosedOrders, handleBuy, continuePaymentOrder, onContinuePayment }) {
  ...
  const sortedProducts = useMemo(() => {
    return [...products]
      .filter((product) => !keyword || isExactIdMatch(product.idCode, keyword))
      ...
      <section className="card">
        <div className="row" style={{ marginBottom: 12 }}>
          <div className="search-box">
            <SearchIcon />
            <input className="input search-input" value={search} onChange={(event) => setSearch(event.target.value.replace(/\D/g, ""))} placeholder="Tìm chính xác ID sản phẩm..." inputMode="numeric" />
          </div>
...
function AdminView({ adminUnlocked, pin, setPin, loginAdmin, products, activeOrders, closedOrders, showAdminClosedOrders, setShowAdminClosedOrders, productForm, setProductForm, handleAddProduct, handleDeleteProduct, handleEditProduct, cancelEditProduct, handleSetProductStatus, handleConfirmPaid, handleCancelOrder, settings, handleUpdatePaymentMinutes, adminProductSearch, setAdminProductSearch, adminScreen, setAdminScreen, handleTogglePackedByPhone, requestDeletePackingOrder, requestDeleteAllPackingOrders }) {
  ...
  const adminVisibleProducts = products.filter((product) => !adminKeyword || isExactIdMatch(product.idCode, adminKeyword));
  ...
  return (
    <div className="admin-grid" style={{ display: "grid", gridTemplateColumns: "minmax(280px, 360px) 1fr", gap: 14 }}>
      <section className="card" style={{ height: "fit-content" }}>
        <button className="btn" style={{ width: "100%", marginBottom: 12 }} onClick={() => setAdminScreen("packing")}>Đóng Hàng</button>
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <section className="card">
          ...
          <div className="row" style={{ marginBottom: 10 }}>
            <div className="search-box">
              <SearchIcon />
              <input className="input search-input" value={adminProductSearch} onChange={(event) => setAdminProductSearch(event.target.value.replace(/\D/g, ""))} placeholder="Tìm chính xác ID sản phẩm..." inputMode="numeric" />
            </div>
...
function PackingView({ packingOrders, onBack, onTogglePacked, onRequestDeleteOrder, onRequestDeleteAll }) {
  ...
  const visiblePackingOrders = useMemo(() => {
    return packingOrders.filter((group) => {
      ...
      const matchId = !normalizedQuery || group.orders.some((order) => isExactIdMatch(order.productCode, normalizedQuery));
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 }}>
        <div className="card" style={{ padding: 8, borderRadius: 12, background: "#f8fafc", borderColor: "#e2e8f0", boxShadow: "0 5px 12px rgba(15,23,42,.03)" }}>
          <p className="muted" style={{ margin: "0 0 2px", fontWeight: 850, fontSize: 10 }}>Đơn chưa đóng</p>
          <p style={{ margin: 0, fontSize: 18, fontWeight: 950, color: "#334155", lineHeight: 1.1 }}>{unpackedCount}</p>
        </div>
        <div className="card" style={{ padding: 8, borderRadius: 12, background: "#f0fdf4", borderColor: "#bbf7d0", boxShadow: "0 5px 12px rgba(15,23,42,.03)" }}>
          <p className="muted" style={{ margin: "0 0 2px", fontWeight: 850, fontSize: 10 }}>Đơn đã đóng</p>
          <p style={{ margin: 0, fontSize: 18, fontWeight: 950, color: "#166534", lineHeight: 1.1 }}>{packedCount}</p>
        </div>
      </div>
        ...
        <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
          <div className="search-box" style={{ width: "100%" }}>
            ...
            <input
              ...
              placeholder="Tìm chính xác ID sản phẩm..."
              inputMode="numeric"
            />
          </div>
          <FilterPills
        ...
        {packingOrders.length === 0 ? (
          ...
          <div className="packing-list">
            {visiblePackingOrders.map((group) => {
              ...
              return (
                <article key={group.phone} className="card" style={{ position: "relative", boxShadow: "none", borderColor: group.packed ? "#bbf7d0" : "#c7d2fe", paddingTop: 46 }}>
                  <button
                    className={statusClass(group.packed ? "packed" : "unpacked")}
                    onClick={() => onTogglePacked(group.phone, !group.packed)}
                    title={group.packed ? "Bấm để chuyển về chưa đóng" : "Bấm để chuyển sang đã đóng"}
                    style={{ position: "absolute", top: 12, right: 12, border: 0, cursor: "pointer" }}
                  >
                    {group.packed ? "Đã đóng hàng" : "Chưa đóng hàng"}
                  </button>

                  <div style={{ minWidth: 0, paddingRight: 8 }}>
                    <p style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 900 }}>IG: {group.buyerIg || "-"}</p>
                    <div className="row" style={{ alignItems: "center", gap: 6, margin: "0 0 6px" }}>
                    ...
                    <p style={{ margin: "0 0 6px", fontWeight: 400 }}>{group.phone || "-"}</p>
